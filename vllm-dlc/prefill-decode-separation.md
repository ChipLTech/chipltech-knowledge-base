...
本文用于理解和验证 DLC Platform 上基于 `MooncakeDLCConnector` 的 Prefill/Decode Separation（PD 分离）：单机 TCP、单机 `lyp_full`、跨机器 TCP，以及 Prefill、Decode、Proxy、KV Cache transfer 和故障边界。术语以 [CONTEXT.md](../CONTEXT.md) 为准；可执行顺序由 `pd-separation` skill 维护。
...
| Topology | `single_node_tcp`、`single_node_lyp_full` 或 `cross_machine_tcp` |
...
2. **Static contract**：两侧 identity、role profile、connector、layout、topology 和 endpoint matrix 闭合。
3. **Read-only preflight**：device/process/memory、link test、operator smoke、ports 和 peer reachability 有明确结果。
4. **Role startup**：Prefill、Decode、Proxy 各有独立 server epoch，launcher/connector/transport 日志符合 contract。
5. **Single request**：通过 Proxy 发一个 deterministic request，关联 client、Proxy、Prefill 和 Decode request identity。
6. **KV transfer**：保存 Prefill block retention/registration、Decode KV pull、block IDs、transport completion 和 DLC-side receive/synchronization。
7. **Functional equivalence**：响应满足声明的 assertion，三个服务 health-after 通过。
8. **Lifecycle/concurrency**：逐步增加请求、并发和 context，每轮只改变一个变量。
9. **Performance**：分别报告 TTFT、TPOT、ITL、throughput、queueing 和 KV-transfer cost。
10. **Cleanup**：只停止 task-owned Proxy、Decode、Prefill，process/port/device memory 回到 sealed baseline。
...
Chip ID 写入、kernel module reload、HBM repair、Bluejay/firmware 初始化、会改变状态的 LYP init、PCIe generation reconfiguration、reboot、`--privileged` + Host PID/system mounts，以及终止非 task-owned process 都是 Host-maintenance action。Device execution 授权不包含这些操作。若异常退出后无法通过 task-scoped graceful cleanup 恢复，记录 `blocked_cleanup_incomplete`，不得自动执行驱动重载或全局进程清理。
