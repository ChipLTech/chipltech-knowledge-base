...
dlc-thunk
LLVM
DLCsim (must use the rebuilt LLVM)
DLCSynapse
DLC_CL
...
LLVM 变化必须使 DLCsim 及其全部下游失效并重建；不得保留 LLVM 前构建的 DLCsim 作为最终产物。每个 component build record 必须绑定本轮 epoch ID、clean/configure command、source/submodule refs、直接 upstream installed artifact hashes 和 produced artifact hashes；任一 upstream hash 或 epoch 不匹配即拒绝 stale cache/output，不得计为 completed。
...
每个 native component 的 completed 条件是：本轮 clean/configure/build epoch 已绑定直接 upstream hashes，task build output 存在、build log terminal success、install log 写入目标、installed target 的 timestamp/SHA-256，以及适用时 `ldd`/`nm`/fresh import。仅发现同名 base library、命中旧 cache 或无 upstream-hash binding 的 no-op build 不构成重编成功。
