...
- 每次执行前先发现并回显 `<KNOWLEDGE_BASE_ROOT>`、`<SKILLS_ROOT>`、实际 skill 路径和将调用的脚本路径；发现多个候选或身份不明确时停止。
...
每日空镜像里的新模型工作必须按 Contract 拆分为以下阶段：

```text
先写 Runtime Qualification Contract
再 model-adaptation 的只读 pre-handoff profile derivation
再 dlc-env-setup + Host Daily Image Runbook，完成 C0/C1a/C1b/required collective
再生成 environment_handoff/v1
最后执行 device-backed model-adaptation
```
...
先发现并回显 `<KNOWLEDGE_BASE_ROOT>`、`<SKILLS_ROOT>`、当前安装的 dlc-env-setup skill 与实际 scripts 路径。然后读取并遵循：
...
5. PyTorch 阶段必须直接运行：
   - `<DLC_ENV_SETUP_SCRIPTS>/pytorch-preflight.sh "$PYTORCH_SRC"`
...
8. 安装后必须直接运行：
   - `<DLC_ENV_SETUP_SCRIPTS>/runtime-smoke.sh /tmp`
...
- <KNOWLEDGE_BASE_ROOT>/CONTEXT.md
- 当前安装的 dlc-env-setup/SKILL.md
- <DLC_ENV_SETUP_SCRIPTS>/vllm-preflight.sh
- <DLC_ENV_SETUP_SCRIPTS>/runtime-smoke.sh
- <KNOWLEDGE_BASE_ROOT>/debugging-workflows/python-build-preflight-for-pytorch-and-vllm.md
...
- 当前已发现的 `dlc-env-setup/SKILL.md`（以当前团队主线为准）
- 当前已发现的 `dlc-env-setup/scripts/pytorch-preflight.sh`
- 当前已发现的 `dlc-env-setup/scripts/vllm-preflight.sh`
- 当前已发现的 `dlc-env-setup/scripts/runtime-smoke.sh`
