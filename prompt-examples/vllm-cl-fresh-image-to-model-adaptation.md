---
prompt_schema: vllm-cl-three-stage-prompt/v2
...
- 所有 artifact destination 必须在 `vllm-cl` 源码树外。
- 阶段 1 输出是阶段 2 的输入证据，不是模型通过结论。阶段 2 只消费明确的 handoff，不得从口头“环境已好”推导任何运行状态。

## 阶段交接 Contract

阶段 1 只有在创建 task-owned daily-image environment 且完成 C1a/C1b 时，才由 Host Daily Image Runbook 按 [模型运行资格与镜像交付 Contract](../vllm-cl/modelzoo-driven-dlc-tyd-image-contract.md) 写 `environment_handoff/v1`。它索引既有 evidence，不新建 PASS 状态。纯 bootstrap、静态检查或未执行 C1b 的阶段 1 只交付 bootstrap report，不能产生 environment handoff。
...
- <KNOWLEDGE_BASE_ROOT>/prompt-examples/vllm-cl-model-adaptation.md
...
3. handoff 合格后，再对该明确新模型做 device-backed vLLM-CL / DLC Platform 适配分析。
...
- `vllm` / `vllm-cl` import 与 package metadata 验证结果。
...
- PyTorch、NumPy bridge、DLC Platform、请求范围内的 `vllm` / `vllm-cl` import 或 runtime smoke 不健康。
