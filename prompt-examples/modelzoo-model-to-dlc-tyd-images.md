---
prompt_schema: modelzoo-model-to-dlc-tyd-images/v4
required_user_inputs:
  - model_name
  - absolute_local_model_path
default_intent: qualification_and_image_delivery
...
默认 intent 是在 runtime qualification 通过后交付 DLC，并在本地能力和授权允许时继续 TYD。该 intent 不等于 build/export/push/Host-maintenance 授权；agent 应先自动生成 target 和 authorization proposal，只在缺少继续所需授权时请求最小增量。

## 可直接复制 Prompt

```md
请按 runtime-first workflow 直接处理该模型并持续到所有 target terminal state。不要要求我预填可自动发现的字段。

模型名称：<MODEL_NAME>
模型目录：<ABSOLUTE_LOCAL_MODEL_PATH>

其余内容全部自动发现或提出：framework、ModelZoo root、DLC/TYD target、ordinary daily base、source/package identities、device/profile、functional assertions、benchmark、ports、artifact root、tags 和 export paths。先复用本机已验证过程资产、已有 immutable images、空闲设备和仍有效的 standing authorization；无法唯一闭合时输出精确 blocker。`qualification_execution`、`task_owned_kill` 和其他 mutation authorization 只在成为继续条件时请求最小增量，不把 target 或 PASS 当作授权。
...
自动发现不授权 network download、clone/fetch、package install、build、qualification execution、task-owned KILL、device execution、privileged Host integration、tar export、registry push、Host maintenance、reset/reboot 或抢占设备。任一成为继续条件时先消费可验证 standing authorization；仍缺失时输出 `blocked_missing_authorization` 和最小授权范围。
```
