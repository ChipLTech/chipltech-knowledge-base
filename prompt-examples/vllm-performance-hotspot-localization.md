...
用于 vLLM-DLC serving 的 decode latency、TTFT、TPOT/ITL 或 throughput 回归。目标是从端到端逐层收敛到最小热点边界，并区分插桩定位证据与无插桩正式 benchmark。

## 可复制 Prompt

```markdown
使用 `/diagnosing-bugs` 定位这个 vLLM-DLC 性能问题。先读取当前知识库的 `CONTEXT.md`、`runtime-debugging/performance-profiling.md`、`runtime-debugging/chipltech-smi-observability.md` 和最近的性能 case study。
